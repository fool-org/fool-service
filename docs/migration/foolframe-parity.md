...
- 2026-07-03: exposed legacy `getmain` at `/api/v1/auth/getmain`
  ...
  body, returns `Token`, legacy `User`, and top-level `TopMenu` items by
  reusing the migrated user-info and top-menu paths. `App` is currently an
  empty legacy shell until migrated login stores app context.
...
- Legacy `getmain` raw-token user/top-menu loading
