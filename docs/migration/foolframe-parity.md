...
- 2026-07-09: legacy `SW_SYS_PROPERTY` Docker schema migration now has
  idempotent repair blocks for model, filter/source/format/sqlcon, and owner
  columns used by the migrated View/data/AppInstall path. The runtime doctor now
  fails if those property metadata columns drift out of the Docker `car_wash`
  schema.
