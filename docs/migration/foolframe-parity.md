...
Status: complete for the defined FoolFrame migration scope as of 2026-07-15.

This document records the current migration state from `../FoolFrame` to `fool-service`.

## Migration Completion Contract

The migration scope is the behavior reachable through the old Web route table,
the public `IDataService` contract, installed metadata, or a configured runtime
module. It is complete when those surfaces have Spring/Java/Vue replacements
and repeatable source, test, Docker, and browser evidence. Obsolete WCF,
Express/Jade/Angular, SQL Server-specific, Thrift-generated, placeholder, and
intentional no-op implementation details are replaced or preserved at their
observable boundary rather than copied source-for-source.

The server inventory and replacement decisions are recorded in
`docs/migration/foolframe-server-assets.md`; frontend assets and every imported
View are recorded in `foolframe-frontend-assets.md` and
`foolframe-view-matrix.md`. The machine-checkable closeout command is:

`python scripts/legacy_migration_contract.py --require-legacy`

No known required migration work remains. A concrete old-source/current-runtime
contradiction, newly reachable unsupported metadata surface, legacy snapshot
change, or failing migration gate reopens this status. Agent configuration,
authorization hardening, and new product work are tracked separately in
`tasks.md` and do not make the FoolFrame migration incomplete.
...
- 2026-07-15: closed the defined FoolFrame migration scope with a versioned,
  machine-checkable completion contract. The adjacent legacy checkout contains
  469 compiled production Server C# sources, 25 registered Express routes, and
  25 public `IDataService` operations; the frontend inventory contains 43 old
  application assets and the runtime catalog contains 118 imported Views.
  `scripts/legacy_migration_contract.py --require-legacy` now compares those
  snapshots with current Spring aliases, Vue page checks, coverage documents,
  and the 118/118 View matrix, and the repository harness fails on drift. The
  full Java 17 Maven reactor, 227 frontend tests and production build, runtime
  doctor, repository harness, and migration contract passed. SQL Server/WCF/
  Thrift/Express/Jade/Angular implementation details are recorded as deliberate
  platform replacements; concrete source/runtime drift reopens migration.
...
The `Partial` wording below describes whether old classes were translated
one-for-one. It does not mean the defined migration scope is incomplete;
`foolframe-server-assets.md` records why platform replacements and unreachable
implementation details are not target requirements.

| FoolFrame project | Compiled C# files | fool-service target | Java main files | Status |
| --- | ---: | --- | ---: | --- |
...
| `SWUA02-SOWAY.ORM.AUTH` | 13 | `fool-auth` / `fool-app-manage` | 38 / 52 | Auth/menu concepts, Docker auth menu relation schema, legacy role/company/department/menu/authorized-user scalar table mappings and collection surfaces, legacy company/department/user empty user-list factories, legacy authorized-user detail factory, legacy typed menu factory top/sub menu filtering wired through the `getsubmenu` AuthItem response, legacy `AuthItem` unsupported getter/no-op setter surface, legacy `RoleFactory` empty shell, legacy auth table-prefix metadata, legacy company/department/subdepartment/role-department app-install table mappings, and seeded `OrderList` auth menu smoke partially migrated |
...
No known required migration work remains under the completion contract above.
The following are post-migration compatibility triggers, not open backlog; they
become migration work only when a configured module, imported View, or concrete
old-source/current-runtime comparison reaches the described surface.
