...
| `Soway.Server` | 150 | `fool-view` | 27 | Partial view/data REST surface, legacy list row-format handling for `ItemEditType.Format` view items, plus Docker-seeded `OrderList` view/data smoke migrated |
...
| `fool-view` | 27 | yes | View/data controllers and adapters, legacy list row-format behavior for `ItemEditType.Format` view items, legacy default auto-view factory behavior, and Docker-seeded `OrderList` view/data smoke |
