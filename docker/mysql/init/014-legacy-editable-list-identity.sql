-- Repair editable legacy lists whose metadata exposes no usable row identity.

DROP PROCEDURE IF EXISTS `ensure_legacy_editable_list_sysids`;

DELIMITER //

CREATE PROCEDURE `ensure_legacy_editable_list_sysids`()
BEGIN
  ...
  DECLARE target_tables CURSOR FOR
    ...
    JOIN `SW_SYS_VIEW` view_definition
     ...
     AND view_definition.`VIEW_CANEDIT` = 1
     AND view_definition.`VIEW_DEFAULT` IS NOT NULL
...
CALL `ensure_legacy_editable_list_sysids`();
DROP PROCEDURE `ensure_legacy_editable_list_sysids`;
...
JOIN `SW_SYS_VIEW` view_definition
 ...
 AND view_definition.`VIEW_CANEDIT` = 1
 AND view_definition.`VIEW_DEFAULT` IS NOT NULL
JOIN information_schema.`COLUMNS` sysid_column
  ON sysid_column.`TABLE_SCHEMA` = DATABASE()
 AND BINARY sysid_column.`TABLE_NAME` = BINARY model.`table_name`
 AND LOWER(sysid_column.`COLUMN_NAME`) = 'sysid'
SET model.`auto_sys_id` = 1
WHERE model.`auto_sys_id` = 0
  AND model.`id_property` IS NULL;
