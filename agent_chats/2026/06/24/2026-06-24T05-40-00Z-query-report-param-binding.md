# Query Report Parameter Binding

## Prompt

- Continue the active goal: bring the environment up with Docker, complete migration against `../FoolFrame`, and use Vue for the frontend.

## Scope

- Compared legacy `../FoolFrame/src/Server/SWDQ01-Soway.Query/SqlScriptFac.cs`.
- Migrated the `GetSql` behavior that binds named `QueryInstance.ReportParams` into the generated query command.
- Adapted the legacy SQL Server named-parameter behavior to the Java JDBC positional-argument path by replacing referenced `@name` tokens with `?` and adding matching values to count/page argument lists.

## Changes

- Added report-parameter binding inside `QuerySqlBuilder` where-clause generation.
- Preserved existing positional filter arguments and pagination argument ordering.
- Added `QuerySqlBuilderTest#pagedSqlBindsLegacyReportParamsReferencedByName`, which verifies `@state` is converted to `?` and the report value is included for both count and page SQL executions.
- Updated `docs/migration/foolframe-parity.md` to record named report-parameter binding in the migrated `fool-query` SQL builder surface.

## TDD

- Red:
  - `docker run --rm -v "$PWD":/workspace -w /workspace -v "$HOME/.m2":/root/.m2 maven:3.9-eclipse-temurin-17 mvn -pl fool-query -am -Dtest=QuerySqlBuilderTest#pagedSqlBindsLegacyReportParamsReferencedByName -DfailIfNoTests=false test`
  - Result: failed as expected because the SQL still contained `@state` instead of `?`.
- Green:
  - Same command.
  - Result: `QuerySqlBuilderTest`: 1 test, 0 failures, 0 errors.

## Verification

- Focused query regression:
  - `docker run --rm -v "$PWD":/workspace -w /workspace -v "$HOME/.m2":/root/.m2 maven:3.9-eclipse-temurin-17 mvn -pl fool-query -am -Dtest=QueryFactoryTest,QueryInsFacTest,QueryReportTest,QueryContextTest,ReportCompareFilterTest,JdbcQueryExecutorTest,QuerySqlBuilderTest,QueryInstanceMigrationTest,QueryLookupCollectionTest,SelectTypeCatalogTest,SelectedColumnCollectionTest,SelectedTablesTest,CompareOpCatalogTest,CompareFilterTest -DfailIfNoTests=false test`
  - Result: 42 tests, 0 failures, 0 errors.
- Full backend package on the Compose network:
  - `docker run --rm --network fool-service_default -e SPRING_DATASOURCE_URL='jdbc:mysql://mysql:3306/car_wash?serverTimezone=Asia/Shanghai&useSSL=false&allowPublicKeyRetrieval=true&characterEncoding=UTF-8&allowMultiQueries=true&autoReconnect=true' -e SPRING_DATASOURCE_USERNAME=root -e SPRING_DATASOURCE_PASSWORD=Pa88word -v "$PWD":/workspace -w /workspace -v "$HOME/.m2":/root/.m2 maven:3.9-eclipse-temurin-17 mvn package`
  - Result: `BUILD SUCCESS` across all 15 backend reactor modules in 24.701 s.

## Runtime Evidence

- Backend Compose image/runtime:
  - `docker compose build -q backend && docker compose up -d backend`: backend image rebuilt and container restarted.
  - `docker compose ps`: backend, frontend, mysql, and redis running; MySQL and Redis healthy.
- Runtime smoke after backend restart:
  - `GET http://localhost:8080/test`: response captured, 132 bytes.
  - `GET http://localhost:8081/`: Vue frontend response captured, 399 bytes.
  - `POST http://localhost:8080/api/v1/data/query-list` with `OrderList` `orderId` filter values `1001,1002`: response captured, 296 bytes.
  - Startup retry observed one transient `curl: (56) Recv failure: Connection reset by peer`; the retry loop continued and all smoke outputs were captured.
- Repository hygiene:
  - `python scripts/check_repo_harness.py`: passed.
  - `git diff --check`: passed.
  - Touched-file trailing whitespace scan: no matches.

## Risks

- The binding only converts report parameters referenced in the generated where SQL. It does not add unused report parameters as extra JDBC args, because extra positional arguments would break Java prepared-statement execution.
- This slice does not complete saved-query/report execution end to end; it only migrates the named report-parameter binding part of the legacy execution path.

## Follow-ups

- Continue remaining saved-query/report execution and richer query-to-view integration parity.
