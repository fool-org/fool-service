# QueryContext CanJoinSelected Parity
...
- Migrated the legacy `CanJoinSelected` context-level state.
- Verified the state defaults to `false`, can be set to `true`, and is preserved by `clear()` because `clear()` only replaces `QueryInstance` in the legacy implementation.
...
- Red test:
  - `docker run --rm -v "$PWD":/workspace -w /workspace -v "$HOME/.m2":/root/.m2 maven:3.9-eclipse-temurin-17 mvn -q -pl fool-query -am -Dtest=QueryContextTest#canJoinSelectedStateBelongsToLegacyContextNotInstance -DfailIfNoTests=false test`
  - Result: test compilation failed because `isCanJoinSelected()` and `setCanJoinSelected(boolean)` did not exist.
- Green focused test:
  - `docker run --rm -v "$PWD":/workspace -w /workspace -v "$HOME/.m2":/root/.m2 maven:3.9-eclipse-temurin-17 mvn -q -pl fool-query -am -Dtest=QueryContextTest#canJoinSelectedStateBelongsToLegacyContextNotInstance -DfailIfNoTests=false test`
  - Result: passed.
- QueryContext regression:
  ...
  - Result: `QueryContextTest`: 9 tests, 0 failures, 0 errors.
